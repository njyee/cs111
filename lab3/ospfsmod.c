...
create_blank_direntry(ospfs_inode_t *dir_oi)
{
	...
	for(pos = 0; pos*OSPFS_DIRENTRY_SIZE < dir_oi->oi_size; pos++) {
		...
		blank = ospfs_inode_data(dir_oi, pos * OSPFS_DIRENTRY_SIZE);
		if(blank != NULL) {
			// got a dir entry
			if(blank->od_ino == 0) // blank
				return blank;
		} else // failed to get dir entry data
...
ospfs_link(struct dentry *src_dentry, struct inode *dir, struct dentry *dst_dentry) {
	...
	// Check if file with link name already exists
	oi = ospfs_inode(dir->i_ino);
	od = find_direntry(oi, dst_dentry->d_name.name, dst_dentry->d_name.len);
	if (od != NULL)
		return -EEXIST;

	// Create new directory entry
	od = create_blank_direntry(oi);
	if (IS_ERR(od))
		return PTR_ERR(od);
	...
	strncpy(od->od_name, dst_dentry->d_name.name, OSPFS_MAXNAMELEN);
...
ospfs_create(struct inode *dir, struct dentry *dentry, int mode, struct nameidata *nd)
{
	...
	if(dentry->d_name.len > OSPFS_MAXNAMELEN)
		return -ENAMETOOLONG;
	// check for file named same as dentry already exists in dir
	if(find_direntry(dir_oi, dentry->d_name.name, dentry->d_name.len) != NULL) 
		return -EEXIST;

	// find an empty inode. Set 'entry_ino' variable to its inode number
	for( entry_ino = 0; entry_ino < ospfs_super->os_ninodes; entry_ino++ ) {
		file_oi = ospfs_inode(entry_ino);
		if(file_oi->oi_nlink == 0) // empty inode has no links
			break;
	}
...
ospfs_symlink(struct inode *dir, struct dentry *dentry, const char *symname)
{
	...
	// Check src and dest name length
	...
	symlink_oi->oi_nlink++;
	strncpy(symlink_oi->oi_symlink, symname, strlen(symname));
